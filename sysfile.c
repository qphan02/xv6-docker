...
sys_chdir(void)
{
	...
	if (setpath(curproc->cid, path, 1) < 0) {
		end_op();
		return -1;
	}
...
name_of_inode(struct inode *ip, struct inode *parent, char buf[DIRSIZ])
{
	...
	for (off = 0; off < parent->size; off += sizeof(de)) {
		if (readi(parent, (char *)&de, off, sizeof(de)) != sizeof(de)) panic("couldn't read dir entry");
		if (de.inum == ip->inum) {
...
name_for_inode(char *buf, int n, struct inode *ip)
{
	...
	if (ip->inum == namei("/")->inum) { // namei is inefficient but iget isn't exported for some reason
		...
		ilock(parent);
		if (name_of_inode(ip, parent, node_name)) { panic("could not find name of inode in parent!"); }
...
sys_getcwd(void)
{
	char *p;
	int   n;
	if (argint(1, &n) < 0 || argptr(0, &p, n) < 0) return -1;
...
int
sys_get_proc_pipe(void)
{
	int *fds;

	if (argptr(0, (void *)&fds, 2 * sizeof(fds[0])) < 0) return -1;

	return get_proc_pipe(fds);
}
int sys_test_0(void) {
	test_0 ();
	return 0;
