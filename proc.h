...
struct proc {
	...
	int 			  priority;
	struct proc * 	  next; 		 // next process in prioqueue
	struct shm_proc_info shms[16];
	int               locks[MUX_MAXNUM]; // -1 by default
	int               ticks;
	int               cid;       // container id
	char              path[128]; // current working path
