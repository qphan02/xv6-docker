...
exec(char *pathcmd, char **argv)
{
	...
	strcpy(path, pathcmd);
	path[strlen(pathcmd)] = 0;
	if (setpath(-1, path, 2) < 0) {
	...
	// remap error
	// for(int k = 0; k<SHM_MAXNUM; k++){
	// 	if(strlen(curproc->shms[k].name) != 0){
	// 		shm_allocuvm(pgdir,sz, sz +PGSIZE, curproc->shms[k].name);
	// 	}
	// }
